    ...
    def draw(self, board, filename, lpath=[], dpath=[]):
        ...
        for r in range(len(board)):
            for c in range(len(board[0])):
                if board[r][c] in self.imgs:
                    img.paste(self.imgs[board[r][c]],
                              (self.res * c, self.res * r))
        for p in lpath:
            row, col = p
            if board[row][col] in self.imgswh:
                img.paste(
                    self.imgswh[board[row][col]
                                ], (self.res * col, self.res * row)
                )
        ...
        for p in dpath:
            ...
            if board[row][col] in self.imgsrh:
                img.paste(
                    self.imgsrh[board[row][col]
                                ], (self.res * col, self.res * row)
                )

        for r in range(len(board)):
            draw.line([(0, r * self.res), (imwidth, r * self.res)],
                      fill="red", width=1)

        for c in range(len(board[0])):
            draw.line(
                [(c * self.res, 0), (c * self.res, imheight)], fill="red", width=1
            )

        img.save(self.dirname + "/" + "Moves" + "/" + filename)
