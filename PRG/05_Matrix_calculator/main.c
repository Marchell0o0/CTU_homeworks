#include <stdio.h>
#include <stdlib.h>

#define INITIAL_CAPACITY 10

enum ErrorCode
{
    ERROR_INPUT = 100,
    ERROR_OUT_OF_MEMORY = 101,
};

struct Matrix
{
    int **data;
    int rows;
    int cols;
};

struct MatrixArray
{
    struct Matrix *matrices;
    char *operations;
    int size;
    int capacity;
    int max_rows;
    int max_cols;
    enum ErrorCode error_code;
};

// input and miscellaneous
void print_error(int return_code);
void print_matrix(struct Matrix matrix);
void check_input_errors(struct MatrixArray *matrix_array);

void copy_matrix(struct Matrix dist, int **src);

// creation of the main array of matrices
struct MatrixArray read_matrices(void);

// main computation
void multiply(struct MatrixArray matrix_array, int first_multiplier, struct Matrix *answer, int c2);
void matrix_add_sub(struct Matrix matrix1, int **matrix2, char opp);

int find_end_of_mult_sequence(struct MatrixArray matrix_array, int start_index);

// memory handling
void free_matrix(struct Matrix matrix);
void free_matrix_array(struct MatrixArray matrix_array);

/* The main program */
int main(int argc, char *argv[])
{
    struct MatrixArray matrix_array = read_matrices();
    if (matrix_array.error_code != EXIT_SUCCESS)
    {
        print_error(matrix_array.error_code);
        free_matrix_array(matrix_array);
        return matrix_array.error_code;
    }

    check_input_errors(&matrix_array);
    if (matrix_array.error_code != EXIT_SUCCESS)
    {
        print_error(matrix_array.error_code);
        free_matrix_array(matrix_array);
        return matrix_array.error_code;
    }

    int max_rows = matrix_array.max_rows;
    int max_cols = matrix_array.max_cols;

    int current_idx = 0;
    int end_of_mult_sequence = find_end_of_mult_sequence(matrix_array, current_idx);

    // create and allocate memory for main answer matrix
    struct Matrix answer;
    answer.rows = matrix_array.matrices[end_of_mult_sequence].rows;
    answer.cols = matrix_array.matrices[end_of_mult_sequence].cols;

    answer.data = malloc(max_rows * sizeof(int *));
    if (!answer.data)
        matrix_array.error_code = ERROR_OUT_OF_MEMORY;

    for (int i = 0; i < max_rows; i++)
    {
        answer.data[i] = malloc(max_cols * sizeof(int));
        if (!answer.data[i])
            matrix_array.error_code = ERROR_OUT_OF_MEMORY;
    }

    int temp_idx;

    copy_matrix(answer, matrix_array.matrices[end_of_mult_sequence].data);
    if (end_of_mult_sequence != 0)
    {
        temp_idx = end_of_mult_sequence;
        while (temp_idx != current_idx)
        {
            temp_idx--;
            multiply(matrix_array, temp_idx, &answer, answer.cols);
        }
    }
    // move current index
    current_idx = end_of_mult_sequence;

    // if something else left
    if (current_idx != matrix_array.size)
    {
        struct Matrix next_matrix;

        next_matrix.data = malloc(max_rows * sizeof(int *));
        if (!next_matrix.data)
            matrix_array.error_code = ERROR_OUT_OF_MEMORY;

        for (int i = 0; i < max_rows; i++)
        {
            next_matrix.data[i] = malloc(max_cols * sizeof(int));
            if (!next_matrix.data[i])
                matrix_array.error_code = ERROR_OUT_OF_MEMORY;
        }

        char current_operation;
        while (current_idx < matrix_array.size)
        {
            current_idx++;
            end_of_mult_sequence = find_end_of_mult_sequence(matrix_array, current_idx);

            next_matrix.rows = matrix_array.matrices[end_of_mult_sequence].rows;
            next_matrix.cols = matrix_array.matrices[end_of_mult_sequence].cols;

            copy_matrix(next_matrix, matrix_array.matrices[end_of_mult_sequence].data);
            temp_idx = end_of_mult_sequence;
            while (temp_idx != current_idx)
            {
                temp_idx--;
                multiply(matrix_array, temp_idx, &next_matrix, next_matrix.cols);
            }

            current_operation = matrix_array.operations[current_idx - 1];
            matrix_add_sub(answer, next_matrix.data, current_operation);

            current_idx = end_of_mult_sequence;
        }
        for (int i = 0; i < max_rows; i++)
            free(next_matrix.data[i]);
        free(next_matrix.data);
    }
    print_matrix(answer);

    for (int i = 0; i < max_rows; i++)
        free(answer.data[i]);
    free(answer.data);

    free_matrix_array(matrix_array);

    return EXIT_SUCCESS;
}

void matrix_add_sub(struct Matrix matrix1, int **matrix2, char opp)
{
    int mult;
    if (opp == '-')
        mult = -1;
    else
        mult = 1;

    for (int i = 0; i < matrix1.rows; i++)
    {
        for (int j = 0; j < matrix1.cols; j++)
            matrix1.data[i][j] += mult * matrix2[i][j];
    }
}

void multiply(struct MatrixArray matrix_array, int first_multiplier, struct Matrix *answer, int c2)
{
    int r1 = matrix_array.matrices[first_multiplier].rows;
    int c1 = matrix_array.matrices[first_multiplier].cols;

    int **temp_answer = (int **)malloc(sizeof(int *) * r1);
    for (int i = 0; i < r1; i++)
        temp_answer[i] = (int *)malloc(sizeof(int) * c2);

    for (int i = 0; i < r1; i++)
    {
        for (int j = 0; j < c2; j++)
        {
            temp_answer[i][j] = 0;
            for (int k = 0; k < c1; k++)
                temp_answer[i][j] += matrix_array.matrices[first_multiplier].data[i][k] * answer->data[k][j];
        }
    }
    answer->rows = r1;
    copy_matrix(*answer, temp_answer);
    for (int i = 0; i < r1; i++)
        free(temp_answer[i]);
    free(temp_answer);
}

int find_end_of_mult_sequence(struct MatrixArray matrix_array, int start_index)
{
    int end_idx = start_index;
    while (end_idx < matrix_array.size && matrix_array.operations[end_idx] == '*')
    {
        end_idx++;
    }
    return end_idx;
}

void copy_matrix(struct Matrix dist, int **src)
{
    for (int i = 0; i < dist.rows; i++)
    {
        for (int j = 0; j < dist.cols; j++)
            dist.data[i][j] = src[i][j];
    }
}

void print_matrix(struct Matrix matrix)
{
    int r = matrix.rows;
    int c = matrix.cols;
    printf("%d %d\n", r, c);
    for (int i = 0; i < r; i++)
    {
        for (int j = 0; j < c; j++)
        {
            printf("%d", matrix.data[i][j]);
            if (j != c - 1)
                printf(" ");
        }
        printf("\n");
    }
}

void print_error(int return_code)
{
    switch (return_code)
    {
    case ERROR_INPUT:
        fprintf(stderr, "Error: Chybny vstup!\n");
        break;
    }
}

void free_matrix(struct Matrix matrix)
{
    for (int i = 0; i < matrix.rows; i++)
    {
        free(matrix.data[i]);
    }
    free(matrix.data);
}

void free_matrix_array(struct MatrixArray matrix_array)
{
    for (int i = 0; i <= matrix_array.size; i++)
    {
        free_matrix(matrix_array.matrices[i]);
    }
    free(matrix_array.matrices);
    free(matrix_array.operations);
}

struct MatrixArray read_matrices(void)
{
    struct MatrixArray matrix_array;
    matrix_array.matrices = malloc(INITIAL_CAPACITY * sizeof(struct Matrix));
    matrix_array.operations = malloc(INITIAL_CAPACITY * sizeof(char));
    matrix_array.size = 0;
    matrix_array.max_rows = 0;
    matrix_array.max_cols = 0;
    matrix_array.capacity = INITIAL_CAPACITY;
    matrix_array.error_code = EXIT_SUCCESS;

    int current_rows, current_cols, correct_inputs;
    char operation;

    while (1)
    {
        // allocate more memory for matrices
        if (matrix_array.size == matrix_array.capacity)
        {
            matrix_array.capacity *= 2;
            matrix_array.matrices = realloc(matrix_array.matrices, matrix_array.capacity * sizeof(struct Matrix));
            matrix_array.operations = realloc(matrix_array.operations, matrix_array.capacity * sizeof(char));
            if (!matrix_array.matrices || !matrix_array.operations)
            {
                matrix_array.error_code = ERROR_OUT_OF_MEMORY;
                break;
            }
        }

        // read size of a matrix
        correct_inputs = scanf("%d %d", &current_rows, &current_cols);

        if (correct_inputs != 2)
        {
            matrix_array.error_code = ERROR_INPUT;
            break;
        }

        if (current_rows > matrix_array.max_rows)
            matrix_array.max_rows = current_rows;

        if (current_cols > matrix_array.max_cols)
            matrix_array.max_cols = current_cols;

        // allocate memory for the matrix
        struct Matrix matrix;
        matrix.rows = current_rows;
        matrix.cols = current_cols;
        matrix.data = malloc(current_rows * sizeof(int *));
        if (!matrix.data)
        {
            matrix_array.error_code = ERROR_OUT_OF_MEMORY;
            break;
        }
        for (int i = 0; i < current_rows; i++)
        {
            matrix.data[i] = malloc(current_cols * sizeof(int));
            if (!matrix.data[i])
            {
                matrix_array.error_code = ERROR_OUT_OF_MEMORY;
                break;
            }
        }

        // read matrix values
        int value;
        for (int i = 0; i < current_rows; i++)
        {
            for (int j = 0; j < current_cols; j++)
            {
                correct_inputs = scanf("%d", &value);
                if (correct_inputs != 1)
                {
                    matrix_array.error_code = ERROR_INPUT;
                }
                matrix.data[i][j] = value;
            }
        }

        matrix_array.matrices[matrix_array.size] = matrix;

        if (matrix_array.error_code != EXIT_SUCCESS)
            return matrix_array;

        scanf("%*c");

        // check if it's the end of the file or an operation
        if (scanf("%c", &operation) == EOF)
            break;
        else
            matrix_array.operations[matrix_array.size] = operation;

        matrix_array.size++;
    }

    return matrix_array;
}

void check_input_errors(struct MatrixArray *matrix_array)
{
    int end_of_mult_sequence;
    int sequence_rows = 0;

    for (int i = 0; i < matrix_array->size; i++)
    {
        int current_rows = matrix_array->matrices[i].rows;
        int current_cols = matrix_array->matrices[i].cols;
        int next_rows = matrix_array->matrices[i + 1].rows;
        int next_cols = matrix_array->matrices[i + 1].cols;
        char operation = matrix_array->operations[i];

        if (operation == '*')
        {
            if (current_cols != next_rows)
            {
                matrix_array->error_code = ERROR_INPUT;
                return;
            }
            if (!sequence_rows)
                sequence_rows = current_rows;
        }
        else
        {
            if (sequence_rows)
            {
                if (i != matrix_array->size - 1 && matrix_array->operations[i + 1] != '*')
                {

                    if (sequence_rows != next_rows || current_cols != next_cols)
                    {
                        matrix_array->error_code = ERROR_INPUT;
                        return;
                    }
                }
                else
                {
                    end_of_mult_sequence = find_end_of_mult_sequence(*matrix_array, i + 1);
                    if (sequence_rows != next_rows || current_cols != matrix_array->matrices[end_of_mult_sequence].cols)
                    {
                        matrix_array->error_code = ERROR_INPUT;
                        return;
                    }
                }
            }
            else
            {
                if (i != matrix_array->size - 1 && matrix_array->operations[i + 1] != '*')
                {
                    if (current_rows != next_rows || current_cols != next_cols)
                    {
                        matrix_array->error_code = ERROR_INPUT;
                        return;
                    }
                }
                else
                {
                    end_of_mult_sequence = find_end_of_mult_sequence(*matrix_array, i + 1);
                    if (current_rows != next_rows || current_cols != matrix_array->matrices[end_of_mult_sequence].cols)
                    {
                        matrix_array->error_code = ERROR_INPUT;
                        return;
                    }
                }
            }
        }
    }
}
