...
my_struct *read_value(void)
{
   ...
   if (str)
   {
      int i = 0;
      if ((n = scanf("%i", &v)) == 1)
      { // reading int has been succesful
         int c = getchar();
         if (c != EOF && isspace(c))
         { // there must be exactly one space between the command and the argument (string/token)
            while ((c = getchar()) != EOF && !isspace(c) && (c >= 0 && c <= 0177) && i < STR_SIZE)
            { // isascii(c)  0-0177
               str[i++] = c;
            }
         }
