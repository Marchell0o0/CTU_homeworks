...
void process_pipe_message(event * const ev, message *msg_to_send)
{
    ...
    switch (msg_received->type){
        ...
        case MSG_GET_VERSION:
            ...
            msg_to_send->data.version.major = 1;
            msg_to_send->data.version.minor = 5;
            msg_to_send->data.version.patch = 2;
            break;
        case MSG_SET_COMPUTE:
            set_compute(msg_received);
            info("Set compute");
            // fprintf(stderr, "DEBUG: Compute is set to this values\nc_re %f c_im %f d_re %f d_im %f n %d\n", chunk.c_re, chunk.c_im, chunk.d_re, chunk.d_im, chunk.n);
            msg_to_send->type = MSG_OK;
            break;
        case MSG_COMPUTE:
            set_chunk(msg_received);
            fprintf(stderr, "INFO: Received chunk id %d\n", msg_received->data.compute.cid);
            // fprintf(stderr, "DEBUG: Chunk is set to this values\nre %f im %f n_re %d n_im %d cid %d\n", chunk.re, chunk.im, chunk.n_re, chunk.n_im, chunk.cid);
